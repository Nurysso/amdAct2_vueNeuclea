...
const telemetryConfig: TelemetryConfig = {
  ...
  serviceName: ${JSON.stringify(serverName)},
  serviceVersion: ${JSON.stringify(serverVersion)},
...
async function startHttpServer() {
  ...
  app.get("/health", (req: Request, res: Response) => {
    res.json({ status: "ok", server: ${JSON.stringify(serverName)} });
  });
  ...
  app.listen(PORT, () => {
    process.stderr.write(\`MCP server "\${${JSON.stringify(serverName)}}" started on http://localhost:\${PORT}\\n\`);
...
async function main() {
  ...
  if (transportType === "stdio") {
    ...
    await server.connect(transport);
    process.stderr.write(\`MCP server "\${${JSON.stringify(serverName)}}" started (stdio)\\n\`);
    if (telemetry.isEnabled) {
      const metricsPort = process.env.METRICS_PORT || '9090';
